require(['jquery','bootstrap-sass','common'],function($){
    $(function(){
        ...
        var globalKey = null;
        var urlGetYzm = [
            '/admin/accountSetting/sendResetPwdAuthCode',
            '/admin/accountSetting/sendOldPhoneAuthCode',
            '/admin/accountSetting/sendResetPayPwdAuthCode'
        ];
        (function(){
        ...
        (function(){
            function urlRequestHandler(){
                ...
                if( $.trim(imgCode.val()).length==6 ){
                    $.ajax({
                        url:'/admin/accountSetting/verifyImgCode',
                        method:'POST',
                        async:false,
                        data:{
            ...
            function changeTel(){
                ...
                function ajaxHandler(){
                    $.ajax({
                        url:'/admin/accountSetting/sendNewPhoneAuthCode',
                        method: 'GET',
                        async:false,
                        data:{
            ...
            function getStaffCode(){
                ...
                $.ajax({
                    url:'/admin/accountSetting/sendOldPhoneAuthCode',
                    method:'GET',
                    async:false,
                    success:function(response){
                        if(response.error=='sendAuthCode_highRate'){
        ...
        (function(){
            ...
            var requestUrl = [
                '/admin/accountSetting/verifyResetPwdAuthCode',
                '/admin/accountSetting/verifyOldPhoneAuthCode',
                '/admin/accountSetting/verifyResetPayPwdAuthCode'
            ];
        ...
        (function(){
            ...
            var requestUrl = ['/admin/accountSetting/resetPwd',null,'/admin/accountSetting/resetPayPwd'];
            var checkPwdEquals= ['/admin/accountSetting/verifyOriginPwdEquals',null,'/admin/accountSetting/verifyOriginPayPwdEquals'];
            ...
            newPwd.on('focus',function(){
                ...
                if( checkPwdHandler(val) ){
                    ...
                    $(this).addClass('error-border');
                    if( !$.trim( $(this).val() ) ){
                        $('#newPwdError').html('6-16个字符、字母、数字、特殊符号；至少两种以上组合；字母区分大小写').show();
                    }else{
                        $('#newPwdError').html('6-16个字符、字母、数字、特殊符号；至少两种以上组合；字母区分大小写').show();
                    }
                    $(this).attr('data-flag','0');
            ...
            $('#telChangeHandler').on('click',function(){
                ...
                $.ajax({
                    url:'/admin/accountSetting/securePhoneExits',
                    method:'POST',
                    data:{
                        phone:$.trim($('#newTel').val())
                    },
                    success:function(response){
                        if(!response){
                            newTelHandler();
                ...
                function newTelHandler(){
                    $.ajax({
                        url:'/admin/accountSetting/verifyNewPhoneAuthCode',
                        method:'POST',
                        data:{
                            newPhone:$.trim(newTel.val()),
                            k:globalKey,
                        ...
                        success:function(response){
                            if( response.success){
                                ...
                                if( response.error == 'Auth_PhoneIllegal' ){
                                    newTel.addClass('error-border').siblings('.right-icon').hide();
                                    $('#newTelError').html('请输入11位数字正确的手机号').show();
                                }else if( response.error == 'Auth_CodeError' || response.error == 'Auth_CodeExpire'){
                                    ...
                                    $('#telVerifycodeError').html('请输入11位数字正确的手机号').show();
                                }
                            }
                        }
                    });
